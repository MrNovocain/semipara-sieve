...
\(\langle \bm a,\bm b\rangle_T=T^{-1}\bm a'\bm b\). The fitted value
\(\bm Q\widehat{\bm c}\) from
\[
...
\(\bm Q'\bm w^c(\Lambda)=\bm0\). These two normal equations are the
finite-sample source of the nuisance annihilation used later: any component in
\(\operatorname{col}(\bm Q)\) is orthogonal to both the profiled residual and
the residualized score weight.
\end{proof}
...
algebra; they are not the organizing principle of the proof. In the one-break
bridge this bookkeeping is collected explicitly in Lemma
\ref{lem:wb-deterministic-bridge-algebra}.
\end{remark}
...
Let
\[
    \D_T=\sigma\{X_{t-1},W_{t-1},w_t:1\leq t\leq T\}.
\]
The inclusion of \(w_t\) is deliberate: the residualized empirical-likelihood
weight \(\bm M_{K,\Lambda}\bm w\) must be fixed in the conditional-design
arguments.  Conditional on \(\D_T\), the variables
\(U_{1,T},\ldots,U_{T,T}\) are
...
\begin{assumption}[Segmentwise orthogonality route for score bias]
\label{ass:score-bias-orthogonality}
For each true segment \(I_{j,0}\), choose \(\bm c_{j,K}\) as the empirical
least-squares projection of \(m_j(W_{t-1})\) on \(\bm P_K(W_{t-1})\) within
that segment, so
\[
    \bm P_{j,0}'\bm r_{j,K}=0.
\]
The instrument admits a segmentwise sieve approximation
\[
    \bm w_{j,0}=\bm P_{j,0}\bm\gamma_{j,K}+\bm s_{j,K},
    \qquad
    \max_j\|\bm s_{j,K}\|_T=O_p(c_K),
\]
where \(\|\cdot\|_T\) uses the full-sample normalization.  The rates satisfy
\[
    a_K\to0,
    \qquad
    \sqrt T\,a_Kc_K\to0.
...
\begin{lemma}[Primitive score-bias verification]
\label{lem:primitive-score-bias}
Under Assumptions \ref{ass:prim-gram}, \ref{ass:prim-sieve} without its first
score-bias display, and \ref{ass:score-bias-orthogonality},
\[
    T^{-1/2}\bm r_0'\bm M_{K,\Lambda_0}\bm w=o_p(1).
\]
Thus the score-bias display in Assumption \ref{ass:prim-sieve} is discharged by
segmentwise empirical projection and a smooth residualized instrument.
\end{lemma}

\begin{proof}[Detailed solution]
On the true partition, \(\bm M_{K,\Lambda_0}\) is block diagonal across the
true segments.  Let \(\bm M_{j,0}\) be the residual-maker for \(\bm P_{j,0}\)
on segment \(j\).  Since \(\bm P_{j,0}'\bm r_{j,K}=0\),
\(\bm M_{j,0}\bm r_{j,K}=\bm r_{j,K}\).  Also
\(\bm M_{j,0}\bm w_{j,0}=\bm M_{j,0}\bm s_{j,K}\).  Hence
\[
    |\bm r_0'\bm M_{K,\Lambda_0}\bm w|
    \leq \sum_{j=0}^{q_0}
    \|\bm r_{j,K}\|\,\|\bm M_{j,0}\bm s_{j,K}\|
    \leq \sum_{j=0}^{q_0}\|\bm r_{j,K}\|\,\|\bm s_{j,K}\|.
\]
Because \(q_0\) is fixed and the norms in Assumptions \ref{ass:prim-sieve} and
\ref{ass:score-bias-orthogonality} use full-sample normalization, the last
sum is \(O_p(Ta_Kc_K)\).  Multiplication by \(T^{-1/2}\) gives
\(O_p(\sqrt T\,a_Kc_K)=o_p(1)\).
\end{proof}
...
\end{assumption}
\begin{assumption}[Primitive sign mass for convex hull]
\label{ass:prim-sign-mass}
There are constants \(c_u,c_v,p_0,\pi>0\) and design-measurable sets
\(J_T\subset\{1,\ldots,T\}\) with \(|J_T|\geq\pi T\) such that
\[
    |v_{t,0}|\geq c_v,
    \qquad t\in J_T,
\]
and, conditionally on \(\D_T\),
\[
    \Pp(U_t>c_u\mid\D_T)\geq p_0,
    \qquad
    \Pp(U_t<-c_u\mid\D_T)\geq p_0,
    \qquad t\in J_T.
\]
For the estimated one-break score, the feasible-score perturbation away from
\(O_p(r_T)\) boundary observations satisfies
\begin{equation}
    a_Kb_K^2
    +b_K\zeta_K\sqrt{\frac{\log\{T(1+r_T)\}}{T}}
    +\frac{b_Kr_T\zeta_K^2}{T}
    \longrightarrow0.
    \label{eq:convex-hull-transfer-rate}
\end{equation}
\end{assumption}

\begin{lemma}[Primitive convex-hull verification]
\label{lem:primitive-convex-hull}
Under Assumptions \ref{ass:prim-stoch}, \ref{ass:prim-bounded},
\ref{ass:prim-gram}, \ref{ass:prim-sieve}, \ref{ass:sol-oracle-envelope}, and
\ref{ass:prim-sign-mass}, Assumption \ref{ass:prim-convex} holds for the true
partition score.  If, in addition, \(q_0=1\),
\(|\widetilde k-k_0|=O_p(r_T)\), and the local bridge rates hold, then
Assumption \ref{ass:prim-convex} also holds for the estimated one-break score.
\end{lemma}

\begin{proof}[Detailed solution]
Condition on \(\D_T\).  Let \(J_T^+=\{t\in J_T:v_{t,0}\geq c_v\}\) and
\(J_T^-=\{t\in J_T:v_{t,0}\leq-c_v\}\).  At least one of these sets has
cardinality at least \(\pi T/2\).  If \(|J_T^+|\geq\pi T/2\), then a positive
oracle score occurs when some \(U_t>c_u\) on \(J_T^+\), and a negative oracle
score occurs when some \(U_t<-c_u\) on \(J_T^+\).  Conditional independence
bounds the probability of missing either sign by \(2(1-p_0)^{\pi T/2}\).  The
case \(|J_T^-|\geq\pi T/2\) is identical with the signs reversed.  For the true partition, the sharper leverage form of the projection-noise bound
and the sieve approximation bound imply a uniform \(o_p(1)\) feasible-score
perturbation on \(J_T\); this is the \(r_T=0\) case of the transfer rate below.
Thus both signs remain present for \(\widehat Z_t(\beta_0,\Lambda_0)\).

For the estimated partition, remove the boundary set where assignments differ;
Lemma \ref{lem:wb-boundary-size} leaves at least \(\pi T-o_p(T)\) observations.
On the remaining observations, the coefficient-update bound, the uniform
projection-noise bound, and the sieve approximation bound give the transfer rate
in \eqref{eq:convex-hull-transfer-rate}.  Hence the estimated score has both
signs with probability tending to one.
\end{proof}
...
\begin{assumption}[Deterministic conditional-design segmentation primitives]
\label{ass:sol-localization}
This is the deterministic part of the route used to discharge the profile-sieve
break search in the one-break main theorem.  Let
\(\bm\mu_K=\bm Q_{K,\Lambda_0}\bm c_0\) be the sieve part of the true nuisance
mean and define the deterministic conditional profile loss
\[
...
\[
    ...
    r_T=o(\sqrt T).
\]
Together with Assumptions \ref{ass:prim-stoch}, \ref{ass:prim-gram}, and
\ref{ass:prim-sieve}, Proposition \ref{prop:bp-primitive-expansion} verifies
the profile-RSS localization and the overfit/underfit RSS gaps recorded in
...
\[
    S_1=\sum_{t=1}^TZ_{t,T}=O_p(\sqrt T),
    \qquad
    S_2=\sum_{t=1}^TZ_{t,T}^2\geq(c+o_p(1))T,
\]
for some \(c>0\).  If \(M_T=\max_{t\leq T}|Z_{t,T}|=o_p(\sqrt T)\), then the
EL multiplier satisfies
\[
...
\[
    ...
    +o_p(T^{-1/2}).
\]
The denominator in this display is the empirical \(L^2(P_T)\) curvature
of the EL KKT equation.
\end{lemma}
...
\(1+\lambda_TZ_{t,T}\) are positive.  Using
\(z/(1+\lambda z)=z-\lambda z^2/(1+\lambda z)\), the KKT equation implies
\[
    S_1
    =
    \lambda_T\sum_{t=1}^T
...
\[
    ...
    |\lambda_T|M_T=o_p(1).
\]
Using \((1+x)^{-1}=1-x+x^2/(1+x)\) in the KKT equation gives
\[
...
\(M_T=\max_t|Z_{t,T}|\).  Lemma \ref{lem:sample-el-local-l2-curvature} gives
\[
    \lambda_T=\frac{S_1}{S_2}+o_p(T^{-1/2}),
    \qquad
    |\lambda_T|M_T=o_p(1).
...
\(\Psi(\widehat\beta_T)=o_p(1)\). Compactness of \(\mathcal B\), continuity of
\(\Psi\), and uniqueness of the population zero imply
...
\begin{theorem}[One-break estimated-partition Wilks]
...
\begin{enumerate}
    ...
    \]
    It also gives the order-selection comparisons
    \[
        RSS_K\{\beta_0,\widehat\Lambda_0(\beta_0)\}
        -RSS_K(\beta_0,k_0)
        \geq cT\Delta_T^2-O_p(R_T),
    \]
    and, uniformly over \(1<q\leq\bar q\),
    \[
        RSS_K(\beta_0,k_0)
...
\begin{proposition}[Primitive verification of the profile-RSS expansion]
\label{prop:bp-primitive-expansion}
Suppose \(q_0=1\), \(\bar q\) is fixed, and Assumptions
\ref{ass:prim-stoch}, \ref{ass:prim-gram}, \ref{ass:prim-sieve}, and
\ref{ass:sol-localization} hold.  Then the uniform RSS expansion and the
underfit/overfit order comparisons in Assumption \ref{ass:bp-profile-sieve}
hold with \(R_T=K+\log T+Ta_K^2\).
\end{proposition}

\begin{proof}[Detailed solution]
Condition on \(\D_T\) and work on the Gram and sieve-approximation events whose
probability tends to one.  For any admissible partition \(\Lambda\), write
\(\bm y(\beta_0)=\bm\mu_K+\bm r_0+\bm U\).  Since
\(\bm M_{K,\Lambda_0}\bm\mu_K=\bm0\),
\begin{align*}
    RSS_K(\beta_0,\Lambda)-RSS_K(\beta_0,\Lambda_0)
    &=D_T(\Lambda)+2\bm U'\bm M_{K,\Lambda}\bm\mu_K
      +\bm U'(\bm M_{K,\Lambda}-\bm M_{K,\Lambda_0})\bm U \\
    &\quad+\mathcal E_{r,T}(\Lambda),
\end{align*}
where \(\mathcal E_{r,T}(\Lambda)\) collects every term containing
\(\bm r_0\).  The admissible search over \(0\le q\le\bar q\) has polynomial
cardinality, so its logarithm is \(O(\log T)\).

For fixed \(\Lambda\), conditional sub-Gaussian tails give
\[
    2|\bm U'\bm M_{K,\Lambda}\bm\mu_K|
    \leq \frac14D_T(\Lambda)+O_p(\log T)
\]
uniformly over the search after a union bound, because the conditional variance
proxy is proportional to \(D_T(\Lambda)\).  Since
\(\bm M_{K,\Lambda}-\bm M_{K,\Lambda_0}
=\bm\Pi_{K,\Lambda_0}-\bm\Pi_{K,\Lambda}\), it is a difference of two
fitted-space projections whose ranks are at most \((\bar q+1)K\).  Hence the
difference has rank \(O(K)\) and operator norm bounded by a constant.
Hanson--Wright concentration gives
\[
    \sup_{\Lambda}
    \left|\bm U'(\bm M_{K,\Lambda}-\bm M_{K,\Lambda_0})\bm U
    -\E\{\bm U'(\bm M_{K,\Lambda}-\bm M_{K,\Lambda_0})\bm U\mid\D_T\}\right|
    =O_p(K+\log T),
\]
and the conditional expectation is \(O(K)\) by the bounded conditional variances
and the same rank bound.

Finally, projection contraction and \(\|\bm r_0\|^2=O_p(Ta_K^2)\) imply,
uniformly over the search,
\[
    |\mathcal E_{r,T}(\Lambda)|
    \leq \frac14D_T(\Lambda)+O_p(Ta_K^2+K+\log T).
\]
The mixed term \(\bm r_0'(\bm M_{K,\Lambda}-\bm M_{K,\Lambda_0})\bm U\) is
absorbed by \(2ab\le a^2+b^2\) and the preceding projection-noise bound.
Combining the three displays yields
\[
    RSS_K(\beta_0,\Lambda)-RSS_K(\beta_0,\Lambda_0)
    \geq \frac12D_T(\Lambda)-O_p(R_T)
\]
uniformly over the admissible search.

For one-break candidates, the deterministic identification inequality in
Assumption \ref{ass:sol-localization} gives the displayed one-break RSS
expansion and the localization rate \(r_T=R_T/\Delta_T^2\).  For the zero-break
model, the underfit identification bound gives
\[
    RSS_K\{\beta_0,\widehat\Lambda_0(\beta_0)\}-RSS_K(\beta_0,k_0)
    \geq cT\Delta_T^2-O_p(R_T).
\]
For \(q>1\), the same uniform lower bound and nonnegativity of \(D_T\) give
\[
    RSS_K(\beta_0,k_0)-RSS_K\{\beta_0,\widehat\Lambda_q(\beta_0)\}=O_p(R_T),
\]
because the right side is the largest possible stochastic overfit gain relative
to the true one-break partition.  These are exactly the order-selection
comparisons in Assumption \ref{ass:bp-profile-sieve}.
\end{proof}
...
\end{proof}
\begin{corollary}[Jointly feasible rates]
\label{cor:jointly-feasible-rates}
Suppose \(\zeta_K^2\asymp K\), \(a_K\asymp c_K\asymp K^{-s/d}\),
\(\Delta_T\geq c_\Delta>0\), and choose
\[
    K\asymp T^{d/(2s+d)}.
\]
If \(s>d\), then
\[
    R_T=K+\log T+Ta_K^2\asymp K,
    \qquad
    r_T=R_T/\Delta_T^2\asymp K,
\]
and all main bridge, local-bias, envelope, convex-hull transfer, and penalty
rates hold with, for example,
\[
    \operatorname{pen}_T(q,K)=qK\log\log(T+e^e).
\]
\end{corollary}

\begin{proof}[Detailed solution]
With \(K\asymp T^{d/(2s+d)}\) and \(a_K\asymp K^{-s/d}\),
\(Ta_K^2\asymp K\), so \(R_T\asymp K\) and \(r_T\asymp K\).  The largest
bridge restriction is
\[
    \frac{K^3\log K}{T}\to0,
\]
up to logarithmic factors, and this follows from \(s>d\).  The local-bias rate
is
\[
    \frac{a_K K^{5/2}}{\sqrt T}\to0,
\]
which is also implied by \(s>d\).  The score-bias orthogonality route requires
\[
    \sqrt T\,a_Kc_K\to0,
\]
which already holds under \(s>d/2\) and therefore under \(s>d\).  The
convex-hull transfer terms reduce to
\[
    K^{1-s/d}\to0,
    \qquad
    K\sqrt{\frac{\log T}{T}}\to0,
    \qquad
    \frac{K^{5/2}}{T}\to0,
\]
again under \(s>d\).  Finally,
\(K\log\log T=o(T\Delta_T^2)\), so the displayed penalty satisfies Assumption
\ref{ass:wb-penalty}.
\end{proof}
...
\end{remark}
\begin{theorem}[Master one-break conditional-design Wilks]
\label{thm:master-one-break-wilks}
Suppose \(q_0=1\), \(\bar q\) is fixed, and Assumptions
\ref{ass:prim-stoch}--\ref{ass:prim-sieve}, \ref{ass:prim-sign-mass},
\ref{ass:sol-oracle-envelope}, \ref{ass:sol-local-bias},
\ref{ass:sol-local-series}, \ref{ass:sol-localization}, and
\ref{ass:wb-penalty} hold.  Suppose also
\[
    T^{-1}\sum_{t=1}^T\sigma_t^2v_{t,0}^2\xrightarrow{p}\eta^2,
    \qquad
    \Pp(\eta\geq\sqrt{c_\eta})=1
\]
for some \(c_\eta>0\), and the bridge rates in Theorem
\ref{thm:wb-one-break-equivalence} hold.  Then, for the fixed one-break
profile estimator \(\widetilde\Lambda=\widehat\Lambda_1(\beta_0)\),
\[
    \ell_T(\beta_0,\widetilde\Lambda)\Rightarrow\chi_1^2.
\]
Moreover, for the unknown-order selector over \(0\le q\le\bar q\) with the
...
\begin{proof}[Detailed solution]
Proposition \ref{prop:bp-primitive-expansion} verifies Assumption
\ref{ass:bp-profile-sieve}.  Theorem \ref{thm:wb-localization} then verifies
the localization input in Assumption \ref{ass:prim-localization-target} for
\(\widetilde\Lambda=\widehat\Lambda_1(\beta_0)\).  Theorem
\ref{thm:wb-one-break-equivalence} then transfers the first two empirical-score
moments from \(\Lambda_0\) to \(\widetilde\Lambda\).  Lemma
\ref{lem:primitive-convex-hull} verifies the convex-hull condition for both the
true and estimated one-break scores under Assumption \ref{ass:prim-sign-mass}
and the displayed transfer rate.  The known-partition Wilks theorem and the
estimated-partition bridge therefore give
\(\ell_T(\beta_0,\widetilde\Lambda)\Rightarrow\chi_1^2\).

For unknown order, Theorem \ref{thm:wb-order} gives
\(\Pp\{\widehat q(\beta_0)=1\}\to1\).  On that event the unknown-order
partition equals \(\widetilde\Lambda\) by the common tie-breaking convention, so
Lemma \ref{lem:wb-event-transfer} transfers the fixed-order Wilks limit to the
...
\begin{assumption}[Local drift stability under break perturbation]
\label{ass:local-drift-stability}
For every fixed \(C<\infty\), uniformly over \(|k-k_0|\leq Cr_T\),
\[
    T^{-1/2}\left|
    \bm X_{lag}'\{\bm M_{K,k}-\bm M_{K,k_0}\}\bm w
    \right|=o_p(1),
\]
where \(q_0=1\).  With
\[
    g_t(k)=\frac{h}{\sqrt T}(\bm M_{K,k}\bm X_{lag})_t
           (\bm M_{K,k}\bm w)_t,
\]
we also have
\[
    \sup_{|k-k_0|\leq Cr_T}
    T^{-1}\sum_{t=1}^T\{g_t(k)-g_t(k_0)\}^2=o_p(1),
    \qquad
    \sup_{|k-k_0|\leq Cr_T}
    \max_t|g_t(k)-g_t(k_0)|=o_p(\sqrt T).
\]
Finally, under \(P_{\beta_T}\), the profile-RSS expansion in Assumption
\ref{ass:bp-profile-sieve} remains valid with the same \(R_T\).
\end{assumption}

\begin{proposition}[Local-law bridge and order stability]
\label{prop:local-law-stability}
Suppose \(q_0=1\), Assumptions \ref{ass:wb-local-oracle} and
\ref{ass:local-drift-stability} hold, and the null-law one-break localization,
bridge, and unknown-order penalty rates hold.  Under \(P_{\beta_T}\),
\[
    |\widetilde k-k_0|=O_p(r_T),
\]
\[
    ...
    \max_t|\widehat Z_t(\beta_0,\widetilde\Lambda)|=o_p(\sqrt T),
\]
and, if unknown order is used,
\[
    P_{\beta_T}\{\widehat q(\beta_0)=1\}\to1.
\]
\end{proposition}

\begin{proof}[Detailed solution]
Under \(\beta_T=\beta_0+h/\sqrt T\), the null-imposed segmentation criterion
adds only the local slope-drift terms generated by \(h\bm X_{lag}/\sqrt T\).
Assumption \ref{ass:local-drift-stability} states precisely that these terms
are \(o_p(1)\) at the empirical-likelihood score scale and are absorbed into the
same \(O_p(R_T)\) RSS remainder used under the null.  Hence the proof of
Theorem \ref{thm:wb-localization} and Theorem \ref{thm:wb-order} applies under
\(P_{\beta_T}\).  The numerator and denominator bridge proofs gain the drift
array \(g_t(k)-g_t(k_0)\), and the two displayed drift-stability bounds make its
contribution negligible in the first two score moments and in the maximum-score
condition.  The remaining terms are exactly the null-law bridge terms already
controlled in Theorem \ref{thm:wb-one-break-equivalence}.
\end{proof}
...
Suppose the conditions of Lemma \ref{lem:wb-local-true-el} hold, Proposition
\ref{prop:local-law-stability} holds, and the local convex-hull condition holds
for \(\widehat Z_t(\beta_0,\widetilde\Lambda)\).  Then
\[
    \ell_T(\beta_0,\widetilde\Lambda)
    \Rightarrow
    \chi_1^2(\delta_h^2).
...
proves the noncentral limit at the true partition from the calibrated local
drift \(g_{t,T}\), the local denominator stability, and the scalar EL expansion.
Proposition \ref{prop:local-law-stability} then moves the result from
\(\Lambda_0\) to \(\widetilde\Lambda\) and supplies local order consistency for
unknown-order transfer.
\end{remark}
...
fixed-\(q_0\) estimated-partition Wilks should extend the proof in the following
order.
\begin{enumerate}
    \item \textbf{Known multiple-break Wilks.}  First keep \(\Lambda_0\) known
    and fixed with fixed \(q_0\).  The existing block-projection algebra already
    covers this case once the Gram, variance, convex-hull, and sieve conditions
    are stated uniformly over the known blocks.

    \item \textbf{Fixed-order estimated bridge.}  Next hold \(q_0\) fixed and
    prove the componentwise bridge
    \[
        \max_{1\le j\le q_0}|\widehat k_j-k_{j,0}|=O_p(r_T),
        \qquad
        S_T(\widehat\Lambda)-S_T(\Lambda_0)=o_p(1),
        \qquad
        V_T(\widehat\Lambda)-V_T(\Lambda_0)=o_p(1).
    \]
    This is where the telescoping display above must become a lemma, including
    the maximum-score condition for the scalar empirical-likelihood expansion.

    \item \textbf{Multi-boundary localization.}  Prove localization under fixed
    \(q_0\), fixed \(\bar q\), minimum spacing, minimum jump size \(\Delta_T\),
    and non-overlapping \(Cr_T\) neighborhoods around the true breaks.  The
    search-uniform stochastic terms should still be of order
    \(R_T=K+\log T+Ta_K^2\) because \(q_0\) is fixed.

    \item \textbf{Unknown-order consistency.}  Use the same penalty
    \[
        \operatorname{pen}_T(q,K)=q\kappa_T R_T.
    \]
    Underfitted models must lose order \(T\Delta_T^2\), while overfitted models
    may gain only \(O_p(R_T)\).  This is the multi-break analogue of Theorem
    \ref{thm:wb-order}.

    \item \textbf{Final multi-break Wilks theorem.}  Only after the fixed-order
    bridge and order-consistency proof are complete should the paper state
    \[
        \ell_T\{\beta_0,\widehat\Lambda(\beta_0)\}\Rightarrow\chi_1^2.
    \]
\end{enumerate}
Until those steps are written as full proofs, multi-break computations are
exploratory diagnostics rather than main-theorem evidence.
...
\section{Closure Checklist After Editor--Author Loop}

The harsh-editor objections addressed in this rebuild are recorded as closure
checks rather than future work.
...
\hline
Editor objection & Author repair & Location \\
\hline
The stochastic environment mixed martingale scores with full-design break search & Replaced it by conditional independence and sub-Gaussian tails given \(\D_T\) & Assumption \ref{ass:prim-stoch} \\
The residualized instrument was not predictable & Made all projected scores design-measurable conditional-on-\(\D_T\) objects & Assumption \ref{ass:prim-score-admissible} \\
The order theorem needed \(R_T=o(T\Delta_T^2)\) and a concrete penalty & Set \(R_T=K+\log T+Ta_K^2\), \(r_T=R_T/\Delta_T^2\), and \(\operatorname{pen}_T(q,K)=q\kappa_TR_T\) & Assumptions \ref{ass:bp-profile-sieve} and \ref{ass:wb-penalty} \\
The underfit RSS gap was implicit & Added an explicit zero-break comparison with \(cT\Delta_T^2-O_p(R_T)\) & Assumption \ref{ass:bp-profile-sieve} \\
The random-index projection bounds were pointwise & Added search-uniform \(T\)-norm and sup-norm projection-noise bounds & Lemma \ref{lem:wb-uniform-projection-noise} \\
The segmentation module still looked asserted rather than verified & Added a conditional-concentration proof of the uniform RSS expansion and order comparisons & Proposition \ref{prop:bp-primitive-expansion} \\
The score-bias condition was still imposed without a primitive route & Added a segmentwise orthogonality and smooth residualized-instrument verification & Assumption \ref{ass:score-bias-orthogonality} and Lemma \ref{lem:primitive-score-bias} \\
Convex hull was a black-box EL condition & Added a primitive sign-mass condition and convex-hull verification lemma & Assumption \ref{ass:prim-sign-mass} and Lemma \ref{lem:primitive-convex-hull} \\
The endpoint theorem was implicit across several modules & Added a master one-break conditional-design Wilks theorem & Theorem \ref{thm:master-one-break-wilks} \\
The rates might be mutually empty & Added a concrete smoothness/dimension regime with \(s>d\) & Corollary \ref{cor:jointly-feasible-rates} \\
Local power reused null-law bridges without saying why & Added local drift stability and local-law bridge/order transfer & Assumption \ref{ass:local-drift-stability} and Proposition \ref{prop:local-law-stability} \\
The estimated-partition theorem overclaimed multiple breaks & Restricted the main result to \(q_0=1\) and moved fixed-\(q_0\) to a telescoping sketch & Section \ref{sec:multiple-break-sketch} \\
\hline
\end{tabular}
\end{center}

\paragraph{Final editorial status.}
Within the declared scope--conditional-on-design errors, one unknown nuisance
break for estimated-partition Wilks, fixed \(\bar q\), and the displayed rate
regime--the workbook now states a coherent proof chain to known-break Wilks,
estimated-break Wilks, and unknown-order transfer.  The local noncentral result
is also stated, but explicitly under the additional local drift-stability and
local convex-hull inputs.  The only material not claimed as a theorem is the
general fixed-multiple-break extension, which is explicitly labeled as an
extension sketch rather than part of the main result.
\end{document}
