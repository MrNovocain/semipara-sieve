...
Suppose \(\zeta_K^2\asymp K\), \(a_K\asymp K^{-s/d}\),
...
\]
which is also implied by \(s>d\).  The convex-hull transfer terms reduce to
\[
    K^{1-s/d}\to0,
    \qquad
...
regime--the workbook now states a coherent proof chain from primitive design
conditions to known-break Wilks, estimated-break Wilks, unknown-order transfer,
and local noncentrality.  The only material not claimed as a theorem is the
