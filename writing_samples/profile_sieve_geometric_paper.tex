...
\begin{equation}
    ...
    \label{eq:block-m}
\end{equation}
Here \({}^\dagger\) denotes the Moore--Penrose inverse. Thus the definition is valid for every candidate partition; when the block Gram matrix is nonsingular, it coincides with the ordinary inverse.
...
Let \(\beta_T=\beta_0+d_T\), where \(d_T\to0\). Define
\[
    g_{t,T}
    =
...
\end{align}
Also assume that, under \(\beta_T\), the feasible true-partition score array satisfies
\[
    P_{\beta_T}\left\{
    \min_{t\leq T}\widehat Z_t(\beta_0,\Lambda_0)<0<
    \max_{t\leq T}\widehat Z_t(\beta_0,\Lambda_0)
...
\[
    ...
    \chi_1^2(\delta_h^2).
\]
If \(P\{\widehat q(\beta_0)=q_0\}\to1\), the same limit holds for \(\ell_T^{BA}(\beta_0)\). The noncentrality parameter is generated by the predictive drift \(d_T\). It does not contain the nuisance-break drift \(B_T\), which is removed by the regime-specific projection.
\end{theorem}
...
Estimated-partition Wilks & Likelihood transfer from the fixed-order estimated score to the true-partition score. & High-level estimated-score equivalence; localization alone is not asserted to prove it, plus consistent \(\widehat q\) for unknown order. \\
\hline
\end{tabular}
\end{center}
