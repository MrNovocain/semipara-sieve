...
\begin{enumerate}
    ...
    \end{equation}
\end{enumerate}
where $Y_t$ represents the asset returns, $W_{t-1}$ is a stationary exogenous weather covariate, and $X_{t-1}$ is a persistent financial predictor. The key exclusion restriction is that the weather process is external to the predictor dynamics: $\{W_s:s\in\mathbb Z\}$ is independent of $X_0$ and of the predictor innovations $\{\varepsilon_t:t\geq1\}$. Hence $W_{t-1}$ is independent of $X_{t-1}$, although $X_t$ may still be endogenous with the return shock through $U_t=\phi\varepsilon_t+z_t$.
...
\begin{enumerate}
    \item \textbf{Exogeneity of Weather Shocks:} Local weather variables ($W_{t-1}$) are determined by atmospheric systems, external to financial variables ($X_{t-1}$) such as trading volume or lagged prices. In the model this means the entire weather path is independent of the initial predictor and its innovations, so $W_{t-1}\perp X_{t-1}$, as formalized in Assumption~\ref{ass:sieve-projection}.
...
\paragraph*{Assumption 5.2 (Independence from the Predictor Process)} \textit{The exogenous weather process is independent of the predictor's initial condition and innovation sequence:}
\begin{equation}
    \{W_s:s\in\mathbb Z\}\ \perp\!\!\!\perp\ \{X_0,\varepsilon_t:t\geq1\}.
\end{equation}
\textit{Since $X_{t-1}$ is generated from $X_0$ and $\{\varepsilon_s:s\leq t-1\}$, this implies $W_{t-1}$ and $X_{t-1}$ are independent random variables. Therefore, for the EL weight $w(X_{t-1})$,}
\begin{equation}
...
Assumption~\ref{ass:ccht-core} holds after enlarging the filtration by the
