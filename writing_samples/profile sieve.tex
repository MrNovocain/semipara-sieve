...
\begin{enumerate}
    \item \textbf{Exogeneity of Weather Shocks:} Local weather variables ($W_{t-1}$) are realized shocks at the daily sampling frequency. They are contemporaneously orthogonal to predetermined financial variables ($X_{t-1}$) such as trading volume or lagged prices, while still being allowed to affect subsequent returns and future prices. This timing restriction is formalized in Assumption~\ref{ass:sieve-projection}.
...
\subsubsection*{Step 1: Profiling out the Weather Curve}
For each candidate parameter $\beta$ under the null hypothesis, we profile out the nonparametric weather curve $m(W_{t-1})$. Let $\bm{Y} = (Y_1, \dots, Y_T)'$ and $\bm{X}_{lag} = (X_0, \dots, X_{T-1})'$. Let $\bm{P}$ be the $T \times K$ matrix whose $t$-th row is $\bm{P}_K(W_{t-1})'$. The profile sieve coefficients for the weather curve are:
\begin{equation}
    \widehat{\bm{c}}_K(\beta) = (\bm{P}'\bm{P})^{-1}\bm{P}'(\bm{Y} - \beta \bm{X}_{lag})
\end{equation}
The residualized dependent variable (the adjusted return) is defined using the sieve annihilator matrix $\bm{M}_K = \bm{I}_T - \bm{P}(\bm{P}'\bm{P})^{-1}\bm{P}'$:
\begin{equation}
...
In standard Empirical Likelihood, the nonparametric estimation error would severely distort the asymptotic distribution. To neutralize this, we construct an orthogonalized weight function. Let $\bm{w} = (w_1, \dots, w_T)'$ where $w_t = w(X_{t-1})$ is the raw weight (e.g., $\tanh(X_{t-1}/c_w)$). We project the weights onto the same sieve basis and define the orthogonalized weight $\bm{w}^c$ as the residual:
\begin{equation}
    \bm{w}^c = \bm{M}_K \bm{w}
\end{equation}
Let $w_t^c = (\bm{w}^c)_t$. By the properties of orthogonal projection, the centered weights are perfectly orthogonal to the sieve basis in-sample:
\begin{equation}
...
\begin{itemize}
    ...
    \item \textbf{Orthogonalization Space}: The true generalization is that the demeaning against a constant $1$ is replaced by residualizing against the Sieve basis $\bm{P}_K(W)$. In Cai et al. (2026), the weight centering $w^c(x) = w(x) - \frac{1}{T}\sum w(X_s)$ represents a projection onto the constant subspace $\text{span}\{\bm{1}\}$. In contrast, our orthogonalized weight $\bm{w}^c$ is projected onto the $K$-dimensional sieve space $\text{span}\{\bm{P}_K(W_{t-1})\}$. We keep the AR(1) equation as the persistence DGP, but we do not use AR(1) OLS residuals in the EL statistic.
\end{itemize}
...
\noindent \textbf{Remark.} This condition says the realized local weather shock is not a proxy for the residual market innovation $U_t$ after conditioning on predetermined market information. It still allows $W_{t-1}$ to affect returns through the structural component $m(W_{t-1})$ and to be incorporated into future prices.

\paragraph*{Assumption 5.2 (Contemporaneous Sieve Orthogonality Between Weather and Predictor Weight)} \textit{At the daily sampling frequency, realized local weather shocks are contemporaneously orthogonal to the predetermined persistent price predictor in the sieve directions used for profiling. For the EL weight $w(X_{t-1})$,}
\begin{equation}
    \mathbb{E}[w(X_{t-1}) \mid W_{t-1}] = \mathbb{E}[w(X_{t-1})].
\end{equation}
\textit{Equivalently, for any centered nonconstant weather sieve vector $\bm R_K(W_{t-1})$,}
\begin{equation}
    \mathbb{E}\left[\bm R_K(W_{t-1})\{w(X_{t-1})-\mathbb{E}w(X_{t-1})\}\right]=\bm0.
\end{equation}
\textit{This is a timing restriction, not a dynamic exclusion: $W_{t-1}$ may affect $Y_t$ through $m(W_{t-1})$ and may affect subsequent returns and future prices.}

\noindent \textbf{Remark.} This assumption is the population version of the sample orthogonality condition used in the proof, $\|T^{-1}\bm R'\widetilde{\bm w}\|=O_p(\sqrt{K/T})$. It preserves the persistent predictor signal at the moment used for inference while allowing weather information to enter later price adjustment.
...
\begin{assumption}[CCHT mean-score conditions]
\label{ass:ccht-core}
The predictor satisfies
\(X_t=\theta+\rho_T X_{t-1}+\varepsilon_t\), where \(\rho_T\) belongs to
one of the persistence classes considered by Cai, Chen, Hong, and Tsvetanov
(CCHT), including the stationary, mildly integrated, local-to-unity,
...
conditional-variance and weight conditions used for the CCHT mean theorem
...
\begin{equation}
    ...
    \label{eq:ccht-core}
\end{equation}
The raw weight is uniformly bounded and satisfies the CCHT saturation
condition.
\end{assumption}

\begin{assumption}[Weather sieve orthogonality and projection]
...
projection of the predictor weight onto the weather sieve.  The
strict-exogeneity condition in Assumption~\ref{ass:ccht-core} holds with
contemporaneous weather information included in the conditioning set.
...
and the martingale moment condition in Assumption~\ref{ass:ccht-core}.
\end{assumption}
...
Under Assumptions~\ref{ass:ccht-core}--\ref{ass:approximation}, under
\(H_0:\beta=\beta_0\),
\begin{align}
    S_T
...
Suppose Assumptions~\ref{ass:ccht-core}--\ref{ass:convex-hull} hold.  Then,
under \(H_0:\beta=\beta_0\),
\begin{equation}
    \ell_T(\beta_0)\ \xrightarrow{d}\ \chi_1^2.
    \label{eq:wilks-result}
\end{equation}
The conclusion holds for each predictor-persistence class covered by the CCHT
mean theorem.
\end{theorem}

\begin{proof}
By Lemma~\ref{lem:score-equivalence} and
Assumption~\ref{ass:ccht-core},
\[
